...
watch(
    filterData,
    (data) => {
        const checkNullObject = Object.values(data).every(value => value === "" || value === null || typeof value === "undefined");
        if(checkNullObject==true){
            currentFilter.value = 'todaysSales'
        }
    },
    {
...
const loadFilters = () => {
    ...
    if (propertyNames.includes('trip')) {
        for (let index = 0; index < salesArray.value.length; index++) {
            const element = salesArray.value[index];
            if (element.tripBatch == filterData.value.trip) {
                finalArray.push(element)
            }
        }
        ...
        filteredSalesArray.value = finalArray
...
const getDesiredName = () => {
    ...
    if (currentFilter.value == 'filteredTripOnly') {
        return filteredUserName.value
    }
}
...
const getDesiredArray = () => {
    ...
    if (currentFilter.value == 'filteredUserOnly' || currentFilter.value == 'filteredTripOnly') {
            ...
            <div class="sm:flex sm:items-center">
                ...
                <div class="sm:flex-auto">
                    <h1 class="text-xl font-semibold text-gray-900">
                        ...
                        <button @click="filtersModal=true"
                            class="capitalize ml-4 inline-flex items-center justify-center rounded-md border border-transparent bg-green-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 sm:w-auto">
                            <i class="fas fa-caret-down mr-4"></i>
                            {{ getDesiredName() }}
                        </button>
                                ...
                                <div class="mt-5 sm:mt-6 sm:grid sm:grid-cols-4 sm:gap-3">
                                    ...
                                    <!-- <div class="sm:col-span-1 mt-2">
                                        <label for="team" class="block text-sm font-medium text-gray-700">Select
                                            Team</label>
                                        <div class="mt-1">
                                            <select :disabled="hideTeam" v-model="filterData.team" id="team" name="team"
                                                ...
                                                <option v-for="agent in saleTeam()" :value="agent" class="font-bold">{{
                                                agent.name }}'s team </option>
                                            </select>
                                        </div>
                                    </div> -->
                                    ...
                                    <!-- <div class="sm:col-span-1 mt-2">
                                        <label for="team" class="block text-sm font-medium text-gray-700">Select
                                            Product</label>
                                        <div class="mt-1">
                                            <select v-model="filterData.product" id="team" name="team"
                                                autocomplete="team-name"
                                                class="font-bold block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm">
                                                <option v-for="product in products" :value="product" class="font-bold">
                                    ...
                                    <!-- <div class="sm:col-span-1 mt-2">
                                        <label for="team" class="block text-sm font-medium text-gray-700">Select
                                            Location</label>
                                        <div class="mt-1">
                                            <select disabled v-model="filterData.location" id="team" name="team"
                                                autocomplete="team-name"
                                                class="font-bold block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm">
                                                <option v-for="agent in saleAgents()" :value="agent" class="font-bold">
                                                    Mexico</option>
                                    ...
                                    <!-- <div class="sm:col-span-1 mt-2">
                                        <label for="team" class="block text-sm font-medium text-gray-700">Select
                                            Driver</label>
                                        <div class="mt-1">
                                            <select disabled v-model="filterData.driver" id="team" name="team"
                                                autocomplete="team-name"
                                                class="font-bold block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm">
                                                <option v-for="agent in saleAgents()" :value="agent" class="font-bold">
                                                    Mexico</option>
                                    ...
                                    <!-- <div class="sm:col-span-1 mt-2">
                                        <label for="team" class="block text-sm font-medium text-gray-700">Select
                                            Payment</label>
                                        <div class="mt-1">
                                            <select disabled v-model="filterData.payment" id="team" name="team"
                                                autocomplete="team-name"
                                                class="font-bold block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm">
                                                <option v-for="agent in saleAgents()" :value="agent" class="font-bold">
                                                    Mexico</option>
                                    ...
                                    <!-- <div class="sm:col-span-1 mt-2">
                                        <label for="team" class="block text-sm font-medium text-gray-700">Select
                                            Client</label>
                                        <div class="mt-1">
                                            <select v-model="filterData.client" id="team" name="team"
                                                autocomplete="team-name"
                                                class="font-bold block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm">
                                                <option v-for="client in clients" :value="client" class="font-bold">{{
